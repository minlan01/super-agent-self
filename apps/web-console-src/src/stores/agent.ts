...
export const useAgentStore = defineStore("agent", () => {
  ...
  async function mergeRuntimeSessionAgents(
    ...
    if (discovered > 0) {
      // Discovered runtime agents from sessions
    }
  ...
  async function fetchAgents() {
    ...
    try {
      ...
      error.value = getErrorMessage(e) || "Failed to load agent list";
    } finally {
      loading.value = false;
    }
  }
  ...
  function setupEventListeners(): void {
    ...
    eventUnsubscribes.push(
      wsStore.subscribe("agent.created", (_agentId: unknown) => {
        fetchAgents().catch((e) => {
          console.warn(
    ...
    eventUnsubscribes.push(
      wsStore.subscribe("agent.deleted", (_agentId: unknown) => {
        fetchAgents().catch((e) => {
          console.warn(
