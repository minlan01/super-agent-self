...
export const useChatStore = defineStore('chat', () => {
  ...
  // 支持多个智能体的状态管理
  // 使用 shallowRef 避免深度代理 Map 内部结构；set 后手动 triggerRef 触发更新
  const agentStatuses = shallowRef<Map<string, AgentStatus>>(new Map())
  const agentSteps = shallowRef<Map<string, AgentStep[]>>(new Map())
  const toolProgress = shallowRef<Map<string, ToolProgress | null>>(new Map())
