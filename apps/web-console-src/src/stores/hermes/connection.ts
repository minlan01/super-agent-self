...
import { HermesApiClient, HermesDashboardUnavailableError } from '@/api/hermes/client'
...
const DEFAULT_HERMES_WEB_URL = import.meta.env.VITE_HERMES_WEB_URL || 'http://localhost:9119'
const DEFAULT_HERMES_API_URL = import.meta.env.VITE_HERMES_API_URL || 'http://localhost:8642'

export type GatewayId = 'openclaw' | 'hermes' | 'myself-agent'

function readStoredGateway(): GatewayId {
  const raw = localStorage.getItem(STORAGE_KEY_GATEWAY)
  if (raw === 'hermes') return 'hermes'
  if (raw === 'myself-agent') return 'myself-agent'
  return 'openclaw'
}
...
function readStoredConnectionConfig(): HermesConnectionConfig {
  ...
  if (!raw) {
    return {
      webUrl: DEFAULT_HERMES_WEB_URL,
      apiUrl: DEFAULT_HERMES_API_URL,
      apiKey: '',
    }
  }
  try {
    ...
    return {
      webUrl: parsed.webUrl || DEFAULT_HERMES_WEB_URL,
      apiUrl: parsed.apiUrl || DEFAULT_HERMES_API_URL,
...
export const useHermesConnectionStore = defineStore('hermes-connection', () => {
  ...
  const currentGateway = ref<GatewayId>(readStoredGateway())
  const hermesConnected = ref(false)
  const hermesConnecting = ref(false)
  const hermesError = ref<string | null>(null)
  const dashboardRequired = ref(false) // Dashboard 不可用标志
   ...
   * 切换网关
   */
  async function switchGateway(gateway: GatewayId) {
    if (gateway === currentGateway.value) return
    if (gateway === 'openclaw' || gateway === 'myself-agent') {
      disconnect()
    }
  ...
  async function connect(): Promise<boolean> {
    ...
    try {
      ...
      if (error instanceof HermesDashboardUnavailableError) {
        dashboardRequired.value = true
        hermesError.value = 'Hermes Web UI (dashboard) is not available. API Server (chat/models) is still available.'
        // 不自动重连（Dashboard 不可用不会自行恢复）
        return false
      }
