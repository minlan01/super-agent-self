...
export const useHermesConfigStore = defineStore('hermes-config', () => {
  ...
  const config = shallowRef<HermesConfig | null>(null)
  const rawConfig = ref('')
  const configSchema = shallowRef<HermesConfigSchema | null>(null)
  const defaults = shallowRef<HermesConfig | null>(null)
  const loading = ref(false)
  const saving = ref(false)
  const lastError = ref<string | null>(null)
  const modifiedFields = shallowRef<Set<string>>(new Set())
