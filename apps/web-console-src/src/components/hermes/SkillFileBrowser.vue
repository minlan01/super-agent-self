...
import { computed, h, onMounted, onUnmounted, ref, shallowRef, nextTick, watch, type VNode } from 'vue'
import {
  NInput,
  NButton,
  NIcon,
...
import { getFileIcon, formatFileSize, getAuthHeaders, imgExts, codeExts, pdfExts } from '@/utils/file-utils'
...
const tocExpandedState = shallowRef<Record<string, boolean>>({})
...
function toggleTocExpand(heading: { text: string; level: number }) {
  const key = getTocKey(heading)
  const next = { ...tocExpandedState.value }
  next[key] = !isTocExpanded(heading)
  tocExpandedState.value = next
}
...
async function browsePath(path: string) {
  ...
  try {
    ...
    entries.value = (data.files || []).map((f: FileEntry) => ({
      name: f.name,
      path: f.path,
    ...
    error.value = getErrorMessage(e) || t('pages.files.loadFailed')
    entries.value = []
  } finally {
    loading.value = false
  }
...
async function openPreview(file: FileEntry) {
  ...
  try {
    ...
    message.error(t('pages.files.readFileFailed') + ': ' + getErrorMessage(e))
  } finally {
...
async function saveFile() {
  ...
  try {
    ...
    message.error(t('pages.files.saveFailed') + ': ' + getErrorMessage(e))
  } finally {
    fileLoading.value = false
  }
}
...
async function createEntry() {
  ...
  try {
    ...
    message.error(t('pages.files.createFailed') + ': ' + getErrorMessage(e))
  } finally {
...
async function deleteEntry(file: FileEntry) {
  try {
    ...
    message.error(t('pages.files.deleteFailed') + ': ' + getErrorMessage(e))
  }
}
...
async function renameEntry() {
  ...
  try {
    ...
    message.error(t('pages.files.renameFailed') + ': ' + getErrorMessage(e))
  } finally {
...
async function handleUpload({ file, onFinish, onError }: { file: UploadFileInfo, onFinish: () => void, onError: (error: Error) => void }) {
  ...
  try {
    ...
    message.error(t('pages.files.uploadFailed') + ': ' + getErrorMessage(e))
