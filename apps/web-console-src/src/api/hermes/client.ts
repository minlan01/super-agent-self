...
/** Dashboard 不可用时抛出的错误 */
export class HermesDashboardUnavailableError extends HermesApiError {
  constructor() {
    super(503, {
      error: 'Hermes Dashboard Unavailable',
      message: 'Hermes Web UI (dashboard) is not available in this version.',
      dashboardRequired: true,
    })
    this.name = 'HermesDashboardUnavailableError'
  }
}
...
export class HermesApiClient {
  ...
  private async request<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (!response.ok) {
      ...
      if (
        typeof body === 'object' &&
        body !== null &&
        (body as Record<string, unknown>).dashboardRequired === true
      ) {
        throw new HermesDashboardUnavailableError()
      }
  ...
  async sendChatStream(
    ...
    sessionId?: string,
    model?: string,
    onToolCall?: (tool: HermesToolCallEvent) => void,
    onDone?: () => void,
  ...
  async getConfigSchema(): Promise<HermesConfigSchema> {
    return this.request<HermesConfigSchema>('/config/schema')
  }
  ...
  async listPlatforms(): Promise<HermesPlatform[]> {
  ...
  async updatePlatform(platformId: string, platformConfig: Record<string, unknown>): Promise<HermesPlatform> {
    const config = await this.getConfig()
    ...
    return { id: platformId, name: platformId, type: platformId, enabled: true, configured: true }
  }

  async createPlatform(platformId: string, platformConfig: Record<string, unknown>): Promise<HermesPlatform> {
    const config = await this.getConfig()
    const platforms = (config.platforms || {}) as Record<string, unknown>
    ...
    return { id: platformId, ...(updatedPlatform || {}) } as HermesPlatform
  }
  ...
  async getMemory(): Promise<HermesMemoryContent> {
    const config = await this.getConfig()
    const memoryData = config.memory as Record<string, unknown> | undefined
    return {
      content: (memoryData?.content as string) || '',
      updatedAt: memoryData?.updatedAt as string | undefined,
      size: ((memoryData?.content as string) || '').length,
    }
  }
