...
export function useConfigEditor(options: UseConfigEditorOptions = {}) {
  ...
  const originalValues = shallowRef<HermesConfig>({})
  const fieldErrors = shallowRef<Map<string, string>>(new Map())
  ...
  function updateField(key: string, value: unknown) {
    configValues.value = { ...configValues.value, [key]: value }
  ...
  function updateFields(updates: Partial<HermesConfig>) {
    configValues.value = { ...configValues.value, ...updates }
  ...
  function resetField(key: string) {
    configValues.value = {
      ...configValues.value,
      [key]: JSON.parse(JSON.stringify(originalValues.value[key])),
    }
    const newErrors = new Map(fieldErrors.value)
  ...
  function validateFieldByKey(key: string): boolean {
    ...
    if (!result.valid) {
      const newErrors = new Map(fieldErrors.value)
      newErrors.set(key, result.error ?? '验证失败')
      fieldErrors.value = newErrors
      return false
    } else {
      const newErrors = new Map(fieldErrors.value)
  ...
  function validate(): boolean {
    ...
    if (!result.valid) {
      ...
      result.errors.forEach(err => {
        newErrors.set(err.key, err.message)
      })
      fieldErrors.value = newErrors
      return false
    }
