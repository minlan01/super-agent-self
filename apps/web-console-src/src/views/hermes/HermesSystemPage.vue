...
const testResult = shallowRef<Record<string, unknown> | null>(null)
...
const envVars = shallowRef<HermesEnvVar[]>([])
...
const configValues = shallowRef<HermesConfig>({})
const originalConfigValues = shallowRef<HermesConfig>({})
...
onMounted(async () => {
  ...
  // Dashboard 不可用时跳过配置加载
  if (connectionStore.dashboardRequired) return
...
<template>
  <div class="hermes-system-page">
    <!-- Dashboard 不可用警告 -->
    <NAlert
      v-if="connectionStore.dashboardRequired"
      type="warning"
      title="Hermes Dashboard 不可用"
      style="margin-bottom: 16px;"
    >
      Hermes CLI v2026.5+ 已移除 Dashboard 子命令，系统配置（Config/Env/Logs）功能暂时不可用。<br />
      可通过 <strong>Hermes CLI</strong> 管理配置，<strong>Chat 页面</strong> 仍可正常使用。
    </NAlert>
