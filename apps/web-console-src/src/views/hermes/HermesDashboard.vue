...
const connectionLabel = computed(() => {
  if (connStore.dashboardRequired) return t('pages.hermesDashboard.connection.dashboardUnavailable') || 'Dashboard Unavailable'
...
const connectionType = computed<'success' | 'warning' | 'error' | 'default'>(() => {
  if (connStore.dashboardRequired) return 'warning'
...
async function fetchUsageAnalytics() {
  if (connStore.dashboardRequired) {
...
async function refreshData() {
  loading.value = true
  try {
    // 如果已知 Dashboard 不可用，跳过连接尝试
    if (!connStore.hermesConnected && !connStore.dashboardRequired) {
      await connStore.connect()
    }

    // Dashboard 不可用时，仅获取 API Server 端的数据 (models)
    // sessions/skills/config/usage 都依赖 Dashboard (port 9119)
    if (connStore.dashboardRequired) {
      await Promise.allSettled([
        modelStore.fetchModels(),
      ])
      // config 不可用时无法同步模型选择，使用 modelStore 自身的当前模型
    } else {
      await Promise.allSettled([
...
<template>
  <NSpin :show="loading">
    <div class="hermes-dashboard">
      ...
      <!-- Dashboard Unavailable Warning -->
      <NAlert
        v-if="connStore.dashboardRequired"
        type="warning"
        title="Hermes Dashboard 不可用"
        style="margin-bottom: 16px;"
      >
        <template #default>
          Hermes CLI v2026.5+ 已移除 Dashboard 子命令。以下功能暂时不可用：
          Sessions（会话管理）、Cron（定时任务）、Skills（技能管理）、Config（配置编辑）、
          Channels（频道管理）、Logs（日志查看）。<br />
          <strong>API Server</strong>（Chat 对话 / Models 模型管理 / CLI 终端）仍可正常使用。
        </template>
      </NAlert>
