...
const originalConfig = ref<MemoryConfig>({})
...
async function fetchConfig() {
  ...
  try {
    ...
    const mem = (config.memory as MemoryConfig) || {}
...
const FileEditor = {
  ...
  props: {
    ...
    state: { type: Object as () => FileEditorState, required: true },
  },
  emits: ['refresh', 'edit', 'cancel', 'save', 'reset'],
  setup(props: { fileName: string; fileDef: Record<string, unknown>; state: FileEditorState }, { emit }: { emit: (event: 'refresh' | 'edit' | 'cancel' | 'save' | 'reset', ...args: unknown[]) => void }) {
    const { t } = useI18n()
