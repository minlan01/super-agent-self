...
import type { HermesProviderConfig, HermesCustomProvider } from '@/api/hermes/types'
import {
  PROVIDER_ID_TO_HERMES_NAME,
  getProviderEnvVar,
  getProviderBaseUrlVar,
  isProviderConfigured as isProviderConfiguredHelper,
  getProviderDisplayApiKey as getProviderDisplayApiKeyHelper,
  getProviderDisplayBaseUrlValue as getProviderDisplayBaseUrlValueHelper,
} from '@/utils/hermes-provider-helpers'
import { useHermesProviderConfig } from '@/composables/useHermesProviderConfig'
...
const customProviders = computed<HermesCustomProvider[]>(() => {
  ...
  if (providers && typeof providers === 'object') {
    const p = providers as unknown as Record<string, unknown>
...
const customProviderFormModels = shallowRef<{ id: string; name?: string }[]>([])
...
const availableModels = shallowRef<{ id: string; name?: string }[]>([])
...
// ---- Provider 配置相关 (composable) ----

const currentModelForConfig = computed(() => currentModelFromConfig.value)
...
const {
  ...
  handleRevealKey,
  handleRevealBaseUrl,
} = useHermesProviderConfig({
  currentModelFromConfig: currentModelForConfig,
  fetchModelsFromEndpoint,
})

// 包装 helpers 以适配页面级调用（传入 store 数据）
function isProviderConfigured(provider: HermesProviderConfig): boolean {
  return isProviderConfiguredHelper(
    provider,
    modelStore.envVars,
...
function getProviderDisplayApiKey(provider: HermesProviderConfig): string {
  return getProviderDisplayApiKeyHelper(
    provider,
    modelStore.envVars,
    modelStore.rawEnvVars as Record<string, { is_set?: boolean; redacted_value?: string }> | null,
    revealedKeys.value,
...
function getProviderDisplayBaseUrlValue(provider: HermesProviderConfig): string {
  return getProviderDisplayBaseUrlValueHelper(
    provider,
    modelStore.envVars,
    modelStore.rawEnvVars as Record<string, { is_set?: boolean; redacted_value?: string }> | null,
    revealedKeys.value,
