...
async function fetchMetrics() {
  ...
  try {
    ...
    error.value = getErrorMessage(e) || t('pages.system.loadFailed')
  } finally {
    loading.value = false
  }
}
            ...
            <NCard size="small" embedded class="metric-card">
              ...
              <div class="metric-content">
                ...
                <div class="metric-stats">
                  ...
                  <NText depth="3" style="font-size: 12px;">
                    {{ formatFileSize(metrics?.memory?.used ?? 0) }} / {{ formatFileSize(metrics?.memory?.total ?? 0) }}
                  </NText>
                </div>
              </div>
            </NCard>
            ...
            <NCard size="small" embedded class="metric-card">
              ...
              <div class="metric-content">
                ...
                <div class="metric-stats">
                  ...
                  <NText depth="3" style="font-size: 12px;">
                    {{ formatFileSize(metrics?.disk?.used ?? 0) }} / {{ formatFileSize(metrics?.disk?.total ?? 0) }}
                  </NText>
                </div>
              </div>
            </NCard>
              ...
              <div class="metric-content">
                <div class="metric-stats metric-stats--full">
                  <div class="network-stat">
                    ...
                    <NText strong>{{ formatFileSize(metrics?.network?.bytesReceived ?? 0) }}</NText>
                  </div>
                  <div class="network-stat">
                    <NText depth="3" style="font-size: 12px;">TX</NText>
                    <NText strong>{{ formatFileSize(metrics?.network?.bytesSent ?? 0) }}</NText>
                  </div>
