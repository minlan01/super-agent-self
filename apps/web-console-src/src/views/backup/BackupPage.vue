...
async function fetchBackupList() {
  try {
    ...
    message.error(getErrorMessage(e) || t('pages.backup.fetchFailed'))
  }
}
...
async function createBackup() {
  ...
  try {
    ...
    message.error(getErrorMessage(e) || t('pages.backup.createFailed'))
  }
}
...
async function downloadBackup(filename: string) {
  try {
    ...
    message.error(getErrorMessage(e) || t('pages.backup.downloadFailed'))
  }
}
...
async function restoreBackup(filename: string) {
  ...
  try {
    ...
    message.error(getErrorMessage(e) || t('pages.backup.restoreFailed'))
  }
}
...
async function deleteBackup(filename: string) {
  try {
    ...
    message.error(getErrorMessage(e) || t('pages.backup.deleteFailed'))
  }
}
...
async function handleUpload({ file }: { file: UploadFileInfo }) {
  ...
  try {
    ...
    message.error(getErrorMessage(e) || t('pages.backup.uploadFailed'))
  }
  return false
}
        ...
        <NGridItem>
          <NCard size="small" embedded>
            ...
            <NText style="font-size: 24px; font-weight: 600;">
              {{ formatFileSize(backupStore.backupList.reduce((sum, b) => sum + b.size, 0)) }}
            </NText>
          </NCard>
        </NGridItem>
        <NGridItem>
