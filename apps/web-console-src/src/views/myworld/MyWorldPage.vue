...
import MuseumToolbar from '@/components/museum/MuseumToolbar.vue'
import MuseumFocusPanel from '@/components/museum/MuseumFocusPanel.vue'
import MuseumAgentList from '@/components/museum/MuseumAgentList.vue'
...
const museumAgentListRef = ref<InstanceType<typeof MuseumAgentList> | null>(null)
...
function onSessionSelected(sessionKey: string) {
  showSessionList.value = false
  showChatPanel.value = true
  chatPanelSessionKey.value = sessionKey
...
watch(() => chatStore.messages, (newMessages, _oldMessages) => {
  // 处理所有智能体的消息事件
...
<template>
  <div class="my-world">
    <OfficeToolbar
      ...
      @create-agent="museumAgentListRef?.openCreateAgentModal()"
      @permission-control="museumAgentListRef?.openToolsModal()"
      @refresh="handleRefresh"
    />
    
    <MuseumToolbar :scale="scale" @zoom-in="handleZoomIn" @zoom-out="handleZoomOut" @reset-view="handleResetView" />
    ...
    <MuseumFocusPanel
      ...
      :selected-character-position="selectedCharacterPosition"
    ...
    <MuseumAgentList ref="museumAgentListRef" @agent-created="onAgentCreated" />
